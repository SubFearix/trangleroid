...
bool proverka(string& str){
	...
	if((simvol + tocka == sizeStr) && (tocka <= 1)){ // Проверяем, чтобы количество цифр + количсетво точек и количество элементов. Также проверяется количество точек
		return true;
	} else {
		return false;
	}
...
double vvodChisla(const string& prompt) {
    ...
    while (true) {
        ...
        if (proverka(input)) { // Проверяем строку
            ...
            cout << "Неверное значение. Пожалуйста, введите целое число." << endl;
        }
    }
}
